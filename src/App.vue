...
const handleAddToCart = (ticketType, movie) => {
  ...
  if (cart.value.some(item => item.movie.id === movie.id && item.ticketType === ticketType)) {
...
const removeFromCart = (index) => {
  console.log(`Removing 1 ${cart.value[index].ticketType} for ${cart.value[index].movie.title}`);
